...
use taiji_validator_node_rpc::client::FinalizedResult;

type JsonObject = json::Map<String, json::Value>;
type CborValue = taiji_bor::Value;

#[derive(Debug, thiserror::Error)]
pub enum JsonEncodingError {
    #[error("Could not decode the substate: {0}")]
    BinaryEncoding(#[from] taiji_bor::BorError),
    #[error("Serde error: {0}")]
...
pub fn cbor_to_json(raw: &[u8]) -> Result<json::Value, JsonEncodingError> {
    ...
    let decoded_cbor: CborValue = taiji_bor::decode(raw)?;
...
pub fn encode_substate_into_json(substate: &Substate) -> Result<json::Value, JsonEncodingError> {
    let substate_cbor = taiji_bor::decode(&substate.to_bytes())?;
...
fn decode_cbor_field_into_json(
    ...
    let cbor_value = taiji_bor::decode(bytes)?;
...
mod tests {
    ...
    use taiji_engine_types::{confidential::ConfidentialOutput, resource_container::ResourceContainer, vault::Vault};
