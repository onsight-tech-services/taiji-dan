...
use taiji_dan_storage::{consensus_models::SubstateRecord, StateStore, StorageError};
use taiji_engine_types::{
    instruction::Instruction,
    substate::SubstateAddress,
    virtual_substate::{VirtualSubstate, VirtualSubstateAddress},
};
use taiji_epoch_manager::{EpochManagerError, EpochManagerReader};
use taiji_indexer_lib::{error::IndexerError, substate_scanner::SubstateScanner};
use taiji_transaction::Transaction;
use taiji_validator_node_rpc::client::{SubstateResult, ValidatorNodeClientFactory};
...
const LOG_TARGET: &str = "taiji::dan::substate_resolver";

#[derive(Debug, Clone)]
pub struct TaijiSubstateResolver<TStateStore, TEpochManager, TValidatorNodeClientFactory> {
    store: TStateStore,
...
impl<TStateStore, TEpochManager, TValidatorNodeClientFactory>
    TaijiSubstateResolver<TStateStore, TEpochManager, TValidatorNodeClientFactory>
where
...
impl<TStateStore, TEpochManager, TValidatorNodeClientFactory> SubstateResolver
    for TaijiSubstateResolver<TStateStore, TEpochManager, TValidatorNodeClientFactory>
where
