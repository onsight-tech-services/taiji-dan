...
use taiji_dan_common_types::{committee::Committee, Epoch, ShardId};
...
use taiji_dan_storage_sqlite::{error::SqliteStorageError, global::SqliteGlobalDbAdapter};
...
const LOG_TARGET: &str = "taiji::dan::committee_state_sync";

pub struct CommitteeStateSync {
    epoch_manager: EpochManagerHandle,
    validator_node_client_factory: TaijiCommsValidatorNodeClientFactory,
...
impl CommitteeStateSync {
    ...
    async fn sync_peers_state(
        ...
        let inventory = inventory
            .into_iter()
            .map(|s| s.to_shard_id())
            .map(taiji_validator_node_rpc::proto::common::ShardId::from)
            .collect::<Vec<_>>();
        ...
        for sync_vn in committee_vns.members {
            ...
            let request = taiji_validator_node_rpc::proto::rpc::VnStateSyncRequest {
