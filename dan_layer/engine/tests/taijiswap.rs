...
struct TaijiSwapTest {
    template_test: TemplateTest,
    a_resource: ResourceAddress,
    b_resource: ResourceAddress,
    lp_resource: ResourceAddress,
    taijiswap: ComponentAddress,
    account_address: ComponentAddress,
    account_proof: NonFungibleAddress,
}

fn setup(fee: u16) -> TaijiSwapTest {
    let mut template_test = TemplateTest::new(vec!["tests/templates/taijiswap", "tests/templates/faucet"]);
    ...
    let (taijiswap, lp_resource) = create_taijiswap_component(&mut template_test, a_resource, b_resource, fee);
...
fn create_taijiswap_component(
    template_test: &mut TemplateTest,
    a_resource: ResourceAddress,
    b_resource: ResourceAddress,
    fee: u16,
) -> (ComponentAddress, ResourceAddress) {
    let module_name = "TaijiSwapPool";
    let taijiswap_template = template_test.get_template_address(module_name);

    let res = template_test
        .execute_and_commit(
            vec![Instruction::CallFunction {
                template_address: taijiswap_template,
...
fn swap(test: &mut TaijiSwapTest, input_resource: &ResourceAddress, output_resource: &ResourceAddress, amount: Amount) {
    test.template_test
        .execute_and_commit(
            vec![
                Instruction::CallMethod {
                ...
                Instruction::CallMethod {
                    component_address: test.taijiswap,
...
fn add_liquidity(test: &mut TaijiSwapTest, a_amount: Amount, b_amount: Amount) {
    test.template_test
        .execute_and_commit(
            vec![
                Instruction::CallMethod {
...
fn remove_liquidity(test: &mut TaijiSwapTest, lp_amount: Amount) {
    test.template_test
        .execute_and_commit(
            vec![
                Instruction::CallMethod {
...
fn get_pool_balance(test: &mut TaijiSwapTest, resource_address: ResourceAddress) -> Amount {
    test.template_test
        .call_method(test.taijiswap, "get_pool_balance", args![resource_address], vec![])
}

fn get_account_balance(test: &mut TaijiSwapTest, resource_address: ResourceAddress) -> Amount {
    test.template_test
        .call_method(test.account_address, "balance", args![resource_address], vec![])
}

fn assert_swap(
    test: &mut TaijiSwapTest,
...
fn assert_add_liquidity(test: &mut TaijiSwapTest, a_amount: i64, b_amount: i64, expected_lp_amount: i64) {
    // create the amount objects
